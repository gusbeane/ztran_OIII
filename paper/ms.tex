...
\title[The vanishing redshift as an EoR probe]{The vanishing redshift as a probe of reionization}

% The list of authors, and the short list which is used in the headers.
% If you need two or more lines of authors, add an extra line using \newauthor
\author[K. Moriwaki et al.]{
Kana Moriwaki,$^{1}$\thanks{E-mail: kana.moriwaki@utap.phys.s.u-tokyo.ac.jp}
Angus Beane,$^{2}$\thanks{E-mail: angus.beane@cfa.harvard.edu}
Adam Lidz,$^{3}$
and others
\\
...
correlated. The redshift at which this occurs (the ``vanishing redshift'') is
...
high redshift. Since the vanishing redshift is derived from a
...
vanishing redshift is measurable up until about $z\sim x$. Being derived from
a cross-correlation, the vanishing redshift is less impacted by foreground
contamination currently inhibiting 21~cm auto-spectrum measurements.
Therefore, the vanishing redshift is an interesting probe of the early stages
...
Because of the difficulties in making 21~cm auto-spectrum measurements, the prospect of measuring 21~cm in cross-correlation with 
...
vanishing redshift is typically $\zv \sim 10$, it is unclear how much
...
this transition occurs at a fixed $k$ as the ``vanishing redshift''
$\zv(k)$, assuming that only one such zero exists. In other words, the
cross-correlation at $k$ vanishes at $z=\zv(k)$.

In this work we investigate to what extent the vanishing redshift is
...
how \zv{} changes as we vary the parameters of the [\OIII] luminosity-mass
relation for a fixed halo catalog.

\section{Methods} \label{sec:methods}
\subsection{The vanishing redshift} \label{ssec:ztran_def}
The vanishing redshift $\zv(k)$ is defined by a field $x$ which traces the
density field on large scales and the condition that
\beq \label{eq:ztran_def}
P_{21,x}(k, \zv(k)) \equiv 0\text{.}
\eeq
If more than one such \zv{} exists, then we choose the lowest one where
...
\subsection{The vanishing redshift} \label{ssec:ztran_results}
...
signal. The vanishing redshift \zv{} (defined where $P_{21,\delta}(\zv{}) =
0$) for each model is shown as a vertically dashed line. We see that models in
which heating is more efficient have an earlier \zv{}.}
\label{fig:aveTb_nf}
\end{figure*}
...
We first investigate the dependence of the vanishing redshift as defined by
the halo field. Fig.~\ref{fig:ztran_vs_k} shows the vanishing redshift
...
Fig.~\ref{fig:ztran_vs_k} shows that \zv{} is only weakly a function of $k$. 

\begin{figure}
\includegraphics[width=\columnwidth]{plots/ztran_vs_k.pdf}
\caption{The vanishing redshift ($\zv$) as defined by cross-correlating with the
...
\cold{}). We see that \zv{} is only a mild function of $k$ for the three
models, with slightly stronger dependence as the heating of the model
decreases. The most dependence comes in the \cold{} model, where \zv{}
...
\caption{The vanishing redshift (\zv) as defined by cross-correlating with
...
see that for the \fid{} and \cold{} models, \zv{} is only mildly a function
...
to resolve a sufficient number of galaxies, the \zv{} is not dependent on the
...
\zv{} typical for the \hot{} model.}
\end{figure}

\subsection{Line-intensity mapping} \label{ssec:lim}
We now turn to understanding how \zv{} depends on the parameters of our
...
In Fig.~\ref{fig:ztran_vs_alpha_sigma} we plot \zv{} as a function of each of
these parameters\,--\,$\alpha$ and $\sigma$. We see that \zv{} is remarkably
...
\caption{The vanishing redshift (\zv) defined by cross-correlating with a
...
respect to $\sigma$, respectively. Clearly \zv{} varies only with the
...
one might consider using to measure \zv{}\,--\,in fact, our entire formalism
...
compute on \zv{}, and so is not an unreasonable choice. We assume a $k$-bin
of fairly large width, from $k=0.1$ to $0.4\,\Mpc^{-1}$.

By expanding $P_{21,\Ha}$ as a linear function about \zv{}, we can estimate the
error on \zv{} by
\beq \label{eq:error_on_zst}
\Var{\zv} = \left(\left.\frac{\partial P_{21,\Ha}}{\partial z}\right|_{\zv}\right)^{-2} \Var{P_{21,\Ha}}\text{,}
\eeq
